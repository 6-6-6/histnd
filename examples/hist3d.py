...
weight = np.random.random(nsize)
# bins
bins = np.arange(101, dtype='float64')/100

# directly call Rust function
histnd.histnd_parallel_f64(c3d_np, [bins]*3, 1, int(nsize/8))

# with a wrapper, simplifies the usage
histnd.histnd_parallel(c3d_np, [bins]*3, int(nsize/8), weight=weight)

# serial version
histnd.histnd_serial(c3d_np, [bins]*3, weight=weight)
